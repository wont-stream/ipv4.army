const langColorsReq = await fetch("https://cdn.jsdelivr.net/gh/anuraghazra/github-readme-stats@master/src/common/languageColors.json")
...
const calculatePercent = (counts: Record<string, number>): Record<string, string> => {
...
const getTopLangs = async (): Promise<Record<string, string>> => {
    const req = await fetch("https://heliopolis.live/api/v1/users/seth/repos?limit=0");
...
export const makeSVG = async (): Promise<string> => {
    ...
    const svgContent = entries.map(([language, percentage], index) => {
